 ...
 * @param periodBaseCount Amount of ticks
 * @return uint8_t 
 */
...
{
    LOG_FUNC("Function call: %s\n", __func__);
    // when we are in TX mode we turn the RX state off and reset the current event.
    sx128x_set_state_rx(dev, sx128x_rx_off);
    sx128x_set_state_event(dev, SX128X_NO_EVENT);
    ...
    switch (dev->settings.packet_type) {
        case SX128X_PACKET_TYPE_LORA:
        {
            sx128x_cmd_set_dio_irq_params(dev, SX128X_IRQ_REG_TX_DONE | SX128X_IRQ_REG_RX_TX_TIMEOUT, 0, 0);
            break;
        }
        default:
    ...
    sx128x_set_state_opmode(dev, SX128X_OPMODE_TX);
}
...
void sx128x_set_state_rx(sx128x_t *dev, sx128x_rx_state_t rx) {
  ...
  switch (rx) {
    case sx128x_rx_off:
        LOG_INFO("set RX state to: off\n");
        dev->state.rx = sx128x_rx_off;
        break;
    case sx128x_rx_listening:
      LOG_INFO("set RX state to: listening\n");
    ...
    case sx128x_rx_receiving:
      ...
      LOG_INFO("set RX state to: receiving\n");
      break;
    case sx128x_rx_received:
      ...
      LOG_INFO("set RX state to: received\n");
      break;
    case sx128x_rx_read:
 ...
 * 
 * @param dev 
 * @param op_mode[possibilities:, SX128X_OPMODE_SLEEP, SX128X_OPMODE_STANDBY, SX128X_OPMODE_SYNTHESIZER_TX, SX128X_OPMODE_TX, SX128X_OPMODE_SYNTHESIZER_RX, SX128X_OPMODE_RX, SX128X_OPMODE_RX_SINGLE, SX128X_OPMODE_RX_CONTINUOUS, SX128X_OPMODE_RX_ACK, SX128X_OPMODE_CAD]
 */
void sx128x_set_state_opmode(sx128x_t *dev, sx128x_opmode_t op_mode)
{
    ...
    case SX128X_OPMODE_TX:
        LOG_DBG_("\n\n\nset op mode: TRANSMITTER WITH TIMOUT %d\n",_sx128x_get_timeout_in_s(3, 2500));
        sx128x_cmd_set_tx(dev, 3, 2500); 
        break;
    ...
    case SX128X_OPMODE_RX:
        LOG_DBG_("\n\n\nset op mode: RECEIVER WITH TIMEOUT %d\n", _sx128x_get_timeout_in_s(3, 2500));
        sx128x_configure_rx(dev);
        _sx128x_cmd_set_rx(dev, 3, 2500); 
        break;
        ...
        case SX128X_PACKET_TYPE_LORA:
            ...
            switch (code_rate) {
                ...
                case LORA_CR_4_8:
                    ...
                    break;
                case LORA_CR_LI_4_5:
                    params[2] = SX128X_LORA_CR_LI_4_5;
                    break;
                case LORA_CR_LI_4_6:
                    params[2] = SX128X_LORA_CR_LI_4_6;
                    break;
                case LORA_CR_LI_4_7:
                    params[2] = SX128X_LORA_CR_LI_4_7;
                    break;
                default:
                    params[2] = SX128X_LORA_CR_4_5;
                    break;
